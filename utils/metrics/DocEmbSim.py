    ...
    def read_data(self, file):
        ...
        with open(file, 'r') as file:
            for line in file:
                text = nltk.word_tokenize(line)
                words.append(text)
        print("Data length:", len(words))
        print("Sample data:", words[:2])  # 打印前兩個樣本，確認數據
        return words

    def generate_batch(self, batch_size, num_skips, skip_window, data):
        assert batch_size % num_skips == 0
        assert num_skips <= 2 * skip_window
        batch = np.zeros(shape=(batch_size), dtype=np.int32)  # 初始化 batch
        labels = np.zeros(shape=(batch_size, 1), dtype=np.int32)  # 初始化 labels
        span = 2 * skip_window + 1  # 上下文窗口的長度
        ...
        while len(buffer) < span:
            if self.data_index < len(data):
                ...
                buffer.append(0)  # 如果數據不足，填充零

        for i in range(batch_size // num_skips):
            target = skip_window  # 目標單詞在窗口的中心
            targets_to_avoid = [skip_window]
            for j in range(num_skips):
                # 避免重複選取 target
                while target in targets_to_avoid:
                    target = random.randint(0, len(buffer) - 1)  # 重新確保 target 在 buffer 範圍內
                targets_to_avoid.append(target)
                batch[i * num_skips + j] = buffer[skip_window]  # 中心單詞
                labels[i * num_skips + j, 0] = buffer[target]  # 上下文單詞
            ...
            if self.data_index < len(data):
                ...
                buffer.append(0)  # 填充零，確保 buffer 長度
