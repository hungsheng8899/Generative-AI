...
def highway(input_, size, num_layers=1, bias=-2.0, f=tf.nn.relu, scope='Highway'):
    ...
    Args:
        ...
        scope (str): 作用域
    Returns:
        Tensor: Highway Network 的輸出
    """
    with tf.variable_scope(scope):
        for idx in range(num_layers):
            with tf.variable_scope(f"highway_layer_{idx}", reuse=tf.AUTO_REUSE):
                g = f(linear(input_, size, scope=f"highway_lin_{idx}"))
                t = tf.sigmoid(linear(input_, size, scope=f"highway_gate_{idx}") + bias)
                output = t * g + (1.0 - t) * input_
                input_ = output  # 更新輸入，傳遞到下一層
    return output  # 返回最後一層的輸出
