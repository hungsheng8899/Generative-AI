...
class Generator(object):
    def __init__(self, num_vocabulary, batch_size, emb_dim, hidden_dim, sequence_length, start_token, learning_rate=0.01, reward_gamma=0.95):
        ...
        self.grad_clip = 5.0
        self.x = tf.compat.v1.placeholder(tf.int32, shape=[self.batch_size, self.sequence_length], name="x")
        ...
        # 定義 g_predictions，先於 build_rl_loss()
        self.build_pretrain()

        # 定義 RL 損失和更新操作
        self.g_loss, self.g_updates = self.build_rl_loss()
    ...
    def generate(self, sess):
        outputs = sess.run(self.gen_x)  # 使用 TensorFlow Session 運行生成過程
        return outputs
    def pretrain_step(self, sess, x):
        ...
        """
        feed = {self.x: x}
        _, g_loss = sess.run([self.pretrain_updates, self.pretrain_loss], feed_dict=feed)
        return _, g_loss
    def build_rl_loss(self):
        """
        定義強化學習的損失函數和更新操作
        """
        with tf.variable_scope('generator_loss'):
            # 選取生成 token 的預測機率
            g_predictions_selected = tf.reduce_sum(
                tf.one_hot(self.x, self.num_vocabulary) * self.g_predictions, axis=2)

            # 計算損失
            g_loss = -tf.reduce_mean(
                tf.reduce_sum(
                    tf.math.log(tf.clip_by_value(g_predictions_selected, 1e-20, 1.0)) *
                    self.rewards, axis=1))

            # 定義更新操作
            g_optimizer = self.g_optimizer(self.learning_rate)
            g_updates = g_optimizer.minimize(g_loss)

            return g_loss, g_updates
